...
const createQuery = (params) => {
  ...
  const searchQuery = `verified #verified -not%20verified -unverified ${city && `#${city} ${city}`} ${material_type} -is:retweet ${resource_type_query}`;
...
const createV1Query = (params) => { 
  ...
  const material_type = "(bed OR beds OR icu OR oxygen OR ventilator OR ventilators OR oxygen%20cylinder OR oxygen%20cylinders OR #bed OR #BED )"; 
  // For V1, -filter: vs -is
  const searchQuery = `verified #verified -not%20verified -unverified ${city && `#${city} ${city}`} ${material_type} -filter:retweet ${resource_type_query}`;
